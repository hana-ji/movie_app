...
class App extends React.Component {
  ...
  getMovies = async () => {
    ...
    // 최신 js라 짧아져야햠 ({ movies(setState의 무비):movies(axios에서 온 무비) }={ movies } 단축해서 사용가능)
  ...
  render() {
    ...
    // 선언해주면 isLoading만 사용해도 가능, movies.map()으로 넣어도되고 renderMovies라는 다른 function 만들어도됨
    return <div>{isLoading ? "Loading..." : movies.map( movies => (
      // .map으로 부터 뭔가를 return 해야함 항상 / Movies + prop 가져와야함
      <Movies key={movies.id} id={movies.id} year={movies.year} title={movies.title} summary={movies.summary} poster={movies.medium_cover_image} />
    ))}</div>
  }
}
